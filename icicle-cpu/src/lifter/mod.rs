...
use crate::{BlockTable, cpu::Arch};

pub use self::{
    optimize::Optimizer,
    pcodeops::{PcodeOpInjector, get_injectors},
    rewriter::read_pc_patcher,
};
...
impl InstructionLifter {
        ...
        S: InstructionSource,
    {
        ...
        let block = self
            ...
            .inspect_err(|e| tracing::trace!("lift-err:   {vaddr:#x}: {e:?}"))?;

        tracing::trace!("pre-rewrite lift:   {vaddr:#x}\n{}", block.display(&src.arch().sleigh));
        rewriter::apply_rewrites(&block, &mut self.lifted);
